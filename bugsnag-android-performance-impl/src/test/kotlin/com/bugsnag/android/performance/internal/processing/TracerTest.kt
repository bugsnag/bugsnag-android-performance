...
class TracerTest {
    ...
    @Test
    fun forceCurrentBatch_NotEmpty() {
        createTracer(emptyList())
        spanFactory = SpanFactory(tracer)
        InternalDebug.spanBatchSizeSendTriggerPoint = 10
        spanFactory.createCustomSpan("Test", SpanOptions.startTime(1L)).end(2L)

        // spanBatchSizeSendTriggerPoint is 10, so end() should NOT wake worker
        verify(worker, times(0)).wake()
