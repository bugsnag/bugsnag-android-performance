...
public interface Sampler {
    public fun sampled(spans: Collection<SpanImpl>): Collection<SpanImpl> {
        return spans.filter { shouldKeepSpan(it) }
    }

    public fun shouldKeepSpan(span: SpanImpl): Boolean
     ...
     */
    public val sampleProbability: Double
        get() = 1.0
}
 ...
 */
@RestrictTo(RestrictTo.Scope.LIBRARY_GROUP)
public object DiscardingSampler : Sampler {
...
public class ProbabilitySampler(
    ...
    override fun shouldKeepSpan(span: SpanImpl): Boolean {
        if (span.category == SpanCategory.APP_SESSION) {
            return true
        }
